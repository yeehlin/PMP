	...
	: CDialog(ElectricModelingDLGMoreMagnetic::IDD, pParent)
{
	...
	for (int i=0;i < 6;i++)
	{
		...
		// 分配共享矩阵指针
		m_pelectricModelingDLGMagnetic[i]->uuu=(float **)((float *)uu + i*3*3);
	}
...
void ElectricModelingDLGMoreMagnetic::SaveSharedData(){
	// 保存输入用于回填
	for (int i = 0;i<this->m_PhaseCount;i++)
	{
		if (m_pelectricModelingDLGMagnetic[i] != NULL){
			...
			for (int m=0;m<3;m++)
			{
				for (int n=0;n<3;n++)
				{
					*((float *)uu + i*3*3+m*3+n) = *(m_pelectricModelingDLGMagnetic[i]->u[m][n]);
				}
			}
		}
