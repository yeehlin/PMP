	...
	: CDialog(ElectricModelingDLGMoreThermal::IDD, pParent)
{
	...
	m_simulationType = 0;
	// 获取共享矩阵c的指针
	kk = (float ***)(ElectricModelingDLGMoreElastic::ther);
	// 初始化指针0
	for (int i=0;i < 6;i++)
	{
		m_pelectricModelingDLGThermal[i] = new ElectricModelingDLGThermal();
		// 分配共享矩阵指针
		m_pelectricModelingDLGThermal[i]->kkk=(float **)((float *)kk + i*3*3);
	}
}
...
void ElectricModelingDLGMoreThermal::SaveSharedData(){
	// 保存输入用于回填
	for (int i = 0;i<this->m_PhaseCount;i++)
	{
		if (m_pelectricModelingDLGThermal[i] != NULL){
			...
			for (int m=0;m<3;m++)
			{
				for (int n=0;n<3;n++)
				{
					*((float *)kk + i*3*3+m*3+n) = *(m_pelectricModelingDLGThermal[i]->k[m][n]);
				}
			}
		}
